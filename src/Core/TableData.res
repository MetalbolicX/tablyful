...
let getColumn = (table: t, name: string): option<column> => {
  table.columns->Array.find(col => col.name === name)
}

let getValue = (row: row, column: string): option<JSON.t> => {
  row->Dict.get(column)
}

let toJson = (table: t): JSON.t => {
  JSON.Encode.object(
    Dict.fromArray([
      ("headers", JSON.Encode.array(table.headers->Array.map(JSON.Encode.string))),
      ("rows", JSON.Encode.array(table.rows->Array.map(row => JSON.Encode.object(row)))),
      (
        "metadata",
        JSON.Encode.object(
          Dict.fromArray([
            ("rowCount", JSON.Encode.float(table.metadata.rowCount->Float.fromInt)),
            ("columnCount", JSON.Encode.float(table.metadata.columnCount->Float.fromInt)),
            ("hasRowNumbers", JSON.Encode.bool(table.metadata.hasRowNumbers)),
            ("sourceFormat", JSON.Encode.string(table.metadata.sourceFormat)),
          ]),
        ),
      ),
    ]),
...
let mapRows = (table: t, f: row => row): t => {
  {...table, rows: table.rows->Array.map(f)}
}

let filterRows = (table: t, f: row => bool): t => {
  let newRows = table.rows->Array.filter(f)
  {
    ...table,
    rows: newRows,
    metadata: {
      ...table.metadata,
      rowCount: newRows->Array.length,
    },
  }
}
